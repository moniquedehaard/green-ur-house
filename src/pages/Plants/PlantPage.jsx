...
export default class PlantPage extends Component {
  ...
  handleClickHomePlant = (id) => {
    ...
    if (user) {
      ...
      const home = user.homePlants.find((el) => el.species === id)
        ? true
        : false;
  ...
  updateButtons(plants) {
    ...
    if (!this.props.user) {
      ...
      plants.forEach((plant) => {
        ...
        const fav = this.props.user.favoritePlants.find(
          (el) => el === plant._id
        )
          ? true
          : false;
        const home = this.state.user.homePlants.find(
          (el) => el.species === plant._id
        )
          ? true
          : false;
                ...
                <div className="card__homePlant" key={el._id}>
                  <div className="card_top">
                    ...
                    <div className="card__buttonblock">
                      {/* Favorite plant */}
                      {(user &&
                        ((buttonValues[el._id].hasFavPlant && (
                          <button
                      ...
                      {(user &&
                        ((buttonValues[el._id].hasHousePlant && (
                          <button
                            onClick={() =>
                              this.handleClickHomePlant(`${el._id}`)
                            }
                            className="btn_round"
                            style={{
                              marginTop: "20px",
                        ...
                        <button
                          onClick={() => this.handleClickHomePlant(`${el._id}`)}
                          className="btn_round"
                          style={{ marginTop: "20px" }}
                        >
                          <i className="fas fa-leaf"></i>{" "}
